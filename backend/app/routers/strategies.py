...
def _compute_needle_under_20(
    target_date, date_str, days, min_score, pattern, bbi_filter, max_drop_pct, long_period, cache_key
) -> Dict:
    """
    单针下二十的CPU密集型计算逻辑（在线程池中执行）
    ...
    # 获取日期范围用于显示
    all_dates = numpy_cache.get_dates_range(days * 2)
    ...
    # 检查数据是否充足
    available_dates = numpy_cache.get_dates_range(50)
    ...
    for check_date in signal_dates:
        for stock_code in numpy_cache.get_all_stocks().keys():
...
async def get_needle_under_20_stocks(  # ✅ 改为async，CPU密集型放到线程池
    ...
    ⚠️ CPU密集型计算使用 asyncio.to_thread 放到线程池，避免阻塞Event Loop
    """
    try:
        # 解析日期
        if date:
        ...
        # ========== CPU密集型计算放到线程池，释放Event Loop ==========
        result = await asyncio.to_thread(
            _compute_needle_under_20,
            target_date, date_str, days, min_score, pattern, bbi_filter, max_drop_pct, long_period, cache_key
        )
        return result
